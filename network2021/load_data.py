...
def reload_single_run(show_plot=True, verbose=False):
    ...
    axs[0, 0].set_title("Neurons", fontsize=title_font_size)
    axs[0, 1].set_title("Neurons", fontsize=title_font_size)
    for i in range(offset, N_total_neurons + offset):
        axs[0, 0].plot(
 ...
 """
    ...
    axs[1, 1].imshow(mus, aspect="auto", interpolation="nearest")
    ...
    axs[2, 1].set_title("Body curvature", fontsize=title_font_size)
    axs[2, 1].imshow(curv_data_less_time, aspect="auto")
    ...
    axs[2, 0].set_title("2D worm motion", fontsize=title_font_size)
    ...
    for t in range(1, tmax, int(tmax / num)):
        ...
        for i in range(point_start, 50):
            ...
            axs[2, 0].plot([x], [y], ".", color=color, markersize=3 if t == 1 else 0.4)
